...
export const MediaUploadForm: React.FC<MediaUploadFormProps> = ({ onSubmit, isLoading, disabled }) => {
  ...
  const commonInputClasses = "w-full p-3 bg-slate-700 border border-slate-600 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none transition-colors placeholder-slate-400 text-slate-100 disabled:opacity-60 disabled:cursor-not-allowed";
  const commonLabelClasses = "block text-sm font-medium text-slate-300 mb-1.5";

  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label htmlFor="mediaFile" className={commonLabelClasses}>
          Upload Image File
        </label>
        <input
          type="file"
          ...
          aria-describedby="mediaFileHelp fileError"
        />
        <button 
            type="button" 
            onClick={triggerFileInput}
            disabled={isLoading || disabled}
            className={`${commonInputClasses} text-left ${selectedFile ? 'text-violet-300' : 'text-slate-400'}`}
        >
            {selectedFile ? selectedFile.name : 'Click to choose an image...'}
        </button>
        <p id="mediaFileHelp" className="text-xs text-slate-500 mt-1">
          Supported types: {SUPPORTED_IMAGE_TYPES.join(', ')}. Max size: {MAX_FILE_SIZE_MB}MB.
        </p>
        {fileError && <p id="fileError" className="text-sm text-red-400 mt-2" role="alert">{fileError}</p>}
      </div>
      ...
      <button
        ...
        className="w-full bg-violet-600 hover:bg-violet-500 text-white font-semibold py-3.5 px-4 rounded-lg transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-violet-400 focus:ring-opacity-75 disabled:bg-slate-600 disabled:hover:bg-slate-600 disabled:cursor-not-allowed transform hover:scale-105 active:scale-100 flex items-center justify-center text-base"
