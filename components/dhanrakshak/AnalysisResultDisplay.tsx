...
import type { FraudAnalysisReport, FraudAssessment } from '../../types'; // Adjusted path
  ...
  return (
    <div className={`mt-6 p-5 bg-slate-800/30 backdrop-blur-sm rounded-xl border-l-4 ${styles.borderColor} shadow-xl animate-fadeIn`}>
      ...
      {potentialRiskFactors && potentialRiskFactors.length > 0 && (
        <div className="mb-1">
          <h3 className="text-sm font-semibold text-slate-300 mb-1.5">Potential Risk Factors:</h3>
      ...
      <p className="text-xs text-slate-500 mt-4 text-right">ID (Mock): {report.transactionId.substring(0,8)}...</p>
    </div>
  );
};
