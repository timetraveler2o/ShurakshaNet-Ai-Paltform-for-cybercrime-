...
import type { UPITransactionData } from '../types';
...
export const TransactionInputForm: React.FC<TransactionInputFormProps> = ({ onSubmit, isLoading, disabled }) => {
  ...
  const commonInputClasses = "w-full p-3 bg-slate-700 border border-slate-600 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-colors placeholder-slate-400 text-slate-100 disabled:opacity-60 disabled:cursor-not-allowed";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <button
        type="submit"
        disabled={isLoading || disabled}
        className="w-full bg-sky-600 hover:bg-sky-500 text-white font-semibold py-3.5 px-4 rounded-lg transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-opacity-75 disabled:bg-slate-600 disabled:hover:bg-slate-600 disabled:cursor-not-allowed transform hover:scale-105 active:scale-100 flex items-center justify-center text-base"
      >
        {isLoading ? (
          <>
